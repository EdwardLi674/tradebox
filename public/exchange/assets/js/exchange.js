    ...
    function tradehistoryupdates() {
        ...
        $.getJSON(BDTASK.getSiteAction('tradehistory?market=' + market), function(data) {
            ...
            $.each(data.tradehistory, function(index, element) {
                ...
                $("#tradeHistory").append("<tr><td class='treade-size " + cls + "'>" + parseFloat(element.complete_qty).toFixed(6)
                                        + "</td><td class='price " + cls + "'>" + parseFloat(element.bid_price).toFixed(6)
                                        + "</td><td class='time'>"+datestring+"</td></tr>");
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data == 0) {
                    ...
                    if (isNaN(trade.balance_up_to) || (trade.balance_up_to == null)) trade.balance_up_to = 0
...
function openOrderUpdate() {
    $.getJSON(BDTASK.getSiteAction('open-order-json'), function(data) {
        ...
        $.each(data.trades, function(index, element) {
            ...
            $("#openOrder").append("<tr><td>" + element.bid_type +
                            "</td><td>" + parseFloat(element.bid_price).toFixed(8) +
                            "</td><td>" + parseFloat(element.bid_qty).toFixed(8) +
                            "</td><td>" + parseFloat(element.bid_qty_available).toFixed(8) +
                            "</td><td>" + parseFloat(element.total_amount).toFixed(8) +
                            "</td><td>" + parseFloat(element.amount_available).toFixed(8) +
                            "</td><td>" + element.market_symbol +
                            "</td><td>" + datestring +
                            ...
                            '</td><td><a href="/order-cancel/'+ element.id +'" class="bg-danger text-white text-center" data-toggle="tooltip" data-placement="left" title="Cancel">Cancel</a>' +
                            "</td></tr>");
        })
    });
}
    ...
    $.getJSON(BDTASK.getSiteAction('complete-trade-json'), function(data) {
        ...
        $.each(data.trades, function(index, element) {
            ...
            var datestring = new Intl.DateTimeFormat('en-AU',{
              ...
              $("#usercompleteTrade").append("<tr><td>" + element.bid_type +
                            "</td><td>" + parseFloat(element.bid_price).toFixed(8) +
                            "</td><td>" + parseFloat(element.bid_qty).toFixed(8) +
                            "</td><td>" + parseFloat(element.total_amount).toFixed(8) +
                            "</td><td>" + element.market_symbol +
...
function userTradeHistoryUpdate() {
    $.getJSON(BDTASK.getSiteAction('user-trade-history-json'), function(data) {
        ...
        $.each(data.trades, function(index, element) {
            ...
            success = new Date(Number(element.unix_success?element.unix_success:element.unix_time))
            var successdatestring = new Intl.DateTimeFormat('en-AU',{
                year: 'numeric', month: 'numeric', day: 'numeric',
                hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: false,
                timeZone: tv.activeChart().getTimezone()
              }).format(success)
            $("#userTradeHistory").append("<tr><td>" + element.bid_type +
                            "</td><td>" + parseFloat(element.bid_price).toFixed(8) +
                            "</td><td>" + parseFloat(element.bid_qty).toFixed(8) +
                            "</td><td>" + parseFloat(element.bid_qty_available).toFixed(8) +
                            "</td><td>" + parseFloat(element.total_amount).toFixed(8) +
                            ...
                            "</td><td>" + element.market_symbol +
                            "</td><td>" + datestring +
                            "</td><td>" + (element.complete_qty==null?'0.0':parseFloat(element.complete_qty).toFixed(8)) +
                            "</td><td>" + (element.complete_amount==null?'0.0':parseFloat(element.complete_amount).toFixed(8)) +
                            "</td><td>" + successdatestring +
                            "</td><td>" + (element.status == 0 ? "<i title='Canceled' class='feather-x-circle text-danger'></i>Canceled" : element.status == 1 ? "<i title='Complete' class='feather-check-circle text-success'></i>Complete" : '<div class="progress"><div title="Running" class="progress-bar progress-bar-striped width100percent">Running</div></div>') +
                            "</td></tr>");
        })
    });
}
